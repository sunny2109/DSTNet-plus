...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['reds', 'gopro', 'dvd', 'bsd', 'set8', 'davis','real', 'compress']:
...
class VideoRecurrentTestDataset(VideoTestDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sigma:
            ...
            noise = torch.normal(0, noise_level.expand_as(imgs_gt))
...
class VideoRecurrentSplitClipsTestDataset(VideoTestDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sigma:
            ...
            imgs_lq = torch.cat([imgs_lq, noise_level.expand(t, 1, h, w)], dim=1)
        else:
            if self.cache_data:
                imgs_lq = self.imgs_lq[folder]
                imgs_gt = self.imgs_gt[folder]
